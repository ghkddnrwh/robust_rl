...
class SACagent(object):
    ...
    def update_target_network(self, TAU):
        ...
        pess_phi_1 = self.pess_critic_1.get_weights()
        pess_phi_2 = self.pess_critic_2.get_weights()
        pess_target_phi_1 = self.pess_target_critic_1.get_weights()
        pess_target_phi_2 = self.pess_target_critic_2.get_weights()
        for i in range(len(pess_phi_1)):
            pess_target_phi_1[i] = TAU * pess_phi_1[i] + (1 - TAU) * pess_target_phi_1[i]
            pess_target_phi_2[i] = TAU * pess_phi_2[i] + (1 - TAU) * pess_target_phi_2[i]
        self.pess_target_critic_1.set_weights(pess_target_phi_1)
        self.pess_target_critic_2.set_weights(pess_target_phi_2)


    ## Q1, Q2 신경망 학습
    def critic_learn(self, state_actions, q_targets, pess_state_actions, pess_q_targets):
        ...
        with tf.GradientTape() as tape:
            pess_q_1 = self.pess_critic_1(pess_state_actions, training=True)
            pess_loss_1 = tf.reduce_mean(0.5 * tf.square(pess_q_1-pess_q_targets))

        pess_grads_1 = tape.gradient(pess_loss_1, self.pess_critic_1.trainable_variables)
        self.pess_critic_1_opt.apply_gradients(zip(pess_grads_1, self.pess_critic_1.trainable_variables))

        with tf.GradientTape() as tape:
            pess_q_2 = self.pess_critic_2(pess_state_actions, training=True)
            pess_loss_2 = tf.reduce_mean(0.5 * tf.square(pess_q_2-pess_q_targets))

        pess_grads_2 = tape.gradient(pess_loss_2, self.pess_critic_2.trainable_variables)
        self.pess_critic_2_opt.apply_gradients(zip(pess_grads_2, self.pess_critic_2.trainable_variables))
    ...
    def actor_learn(self, states):
        ...
        with tf.GradientTape() as tape:
            pess_mu, pess_std = self.pess_actor(states, training=True)
            _, pess_actions, pess_log_pdfs = self.pess_actor.sample_normal(pess_mu, pess_std)
            pess_log_pdfs = tf.squeeze(pess_log_pdfs, 1)
            
            pess_soft_q_1 = self.pess_critic_1(tf.convert_to_tensor(tf.concat([states, pess_actions], axis = -1), dtype=tf.float32))
            pess_soft_q_2 = self.pess_critic_2(tf.convert_to_tensor(tf.concat([states, pess_actions], axis = -1), dtype=tf.float32))
            pess_soft_q = tf.math.minimum(pess_soft_q_1, pess_soft_q_2)

            pess_loss = tf.reduce_mean(self.ALPHA * pess_log_pdfs - pess_soft_q)

        pess_grads = tape.gradient(pess_loss, self.pess_actor.trainable_variables)
        self.pess_actor_opt.apply_gradients(zip(pess_grads, self.pess_actor.trainable_variables))

    ## 시간차 타깃 계산
    def q_target(self, rewards, q_values, dones, r_values):
        y_k = np.asarray(q_values)
        for i in range(q_values.shape[0]): # number of batch
            if dones[i]:
                y_k[i] = rewards[i]
            else:
                y_k[i] = rewards[i] + self.GAMMA * (self.R * r_values + (1 - self.R) * q_values[i])
        return y_k
        ...
        for current_step in range(total_steps):
            ...
            if current_step >= self.UPDATE_AFTER and current_step % self.UPDATE_EVERY == 0:
                for _ in range(self.UPDATE_EVERY):
                    ...
                    pess_target_qs_1 = self.pess_target_critic_1(tf.convert_to_tensor(tf.concat([pess_next_states, pess_next_actions], axis = -1), dtype = tf.float32))
                    pess_target_qs_2 = self.pess_target_critic_2(tf.convert_to_tensor(tf.concat([pess_next_states, pess_next_actions], axis = -1), dtype = tf.float32))
                    pess_target_qs = tf.math.minimum(pess_target_qs_1, pess_target_qs_2)
